...
$(function () {
  ...
  document.onkeyup = function (data) {
    if (data.which == 27) {
      $.post("https://m-Gangs/exit", JSON.stringify({}));
      $counter.val(1);
      $incButton.off("click");
      $decButton.off("click");
      return;
...
async function loadCraftingTable(tableName) {
  const response = await fetch("https://m-Gangs/items", {
    method: "POST",
    headers: {
  ...
  data.forEach((item) => {
    ...
    const classList = ["p-3", "bg-gray-200", "h-[150px]"];
    itemElement.classList.add(...classList);
    itemElement.innerHTML = `<div class="flex items-start justify-between"> <div id="craftable-item" data-location="${item.location}" data-item="${item.item}" class="w-32 h-32 bg-gray-200 border-radius-25 border border-gray-200 flex items-center justify-center relative hover:cursor-pointer hover:bg-gray-800"><div class="text-sm text-black absolute top-1 left-2 line-clamp-1">${item.name}</div><img class="w-16 h-16" src='nui://${item.image}' alt=""><div class="text-sm absolute bottom-1 right-2 text-black" id='current-weight'>${item.itemWeight.toString()}</div><div class="text-sm absolute bottom-1 left-2 text-black" id='current-amount' data-baseamount="${item.craftableAmount}">${item.craftableAmount.toString()}x</div></div><div id="requiredItems" class="flex flex-col items-start w-full gap-1 flex-1 pl-4"></div></div>`;
    ...
    item.itemsNeededToCraft.forEach((requiredItem) => {
      ...
      requiredItemElement.innerHTML = `<div class="flex gap-2"><img class="w-6 h-6" src='nui://${Config.inventory}/html/images/${requiredItem.item}.png' alt=""><div class="text-sm text-black" id='material-count' data-baseweight="${requiredItem.amount}">${requiredItem.item}: ${requiredItem.amount.toString() * parseInt($counter.val())}</div></div>`;
